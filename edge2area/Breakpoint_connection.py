...
def breakpoint_connect(mask_skeleton):  # 输入的是细化后的图像 result1结果
    ...
    while True:  # 找出最近邻断点 连线 删除断点
        if len(breakpoint_loc[0]) <= 1:
            break
        a, b = min_dist2(breakpoint_loc)  # 在这个点集中距离最近的两点，提取为a,b，是下标
...
def connecting(input, output):  # 原本的main  函数化
    # 单一图像
    # src_path=os.path.join(src_path,'output_canong3_kodakdcs330_sub_06.tif')
    # pred_mask=Image.open(src_path)
    ...
    # _mask_after_connect = np.where(_mask_after_connect, 1, 0)
    '''可视化'''
    # plt.figure(1)
    # plt.imshow(_mask_after_connect)
    # plt.show()
    # 多个图像
    src_path = input
    save_path = output
    for index, item in enumerate(os.listdir(src_path)):
        ...
        try:  # 有断点的情况
            ...
            # plt.imshow(_mask_after_connect)
            # plt.show()
            # _mask_after_connect = _mask_after_connect[1:-1][1:-1]
            # print(_mask_after_connect.shape)
